import { CategoryEntity, CategoryRepository, CustomError, UpdateCategoryDto } from "../../";
...
export class UpdateCategory implements UpdateCategoryUseCase {
  ...
  async execute(updateCategoryDto: UpdateCategoryDto): Promise<CategoryEntity> {
    const currentCategory = await this.categoryRepository.findById(updateCategoryDto.id);
    ...
    const updatedCategory = await this.categoryRepository.update(updateCategoryDto);
    if (!updatedCategory) throw CustomError.notFound('Failed to update category');
