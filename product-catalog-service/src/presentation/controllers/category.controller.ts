...
import { CategoryRepository, CreateCategory, CreateCategoryDto, DeleteCategory, FindCategories, FindCategory, UpdateCategory, UpdateCategoryDto } from "../../domain";
...
export class CategoryController {
  ...
  updateCategory = (req: Request, res: Response) => {
    const { id } = req.params;
    const { errors, validatedData } = UpdateCategoryDto.update({ ...req.body, id });
    if (errors) { res.status(400).json({ errors }); return; }
